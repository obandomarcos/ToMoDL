...
class MoDLReconstructor(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.optimizer_dict["optimizer_name"] == "Adam":
            optimizer = torch.optim.Adam(self.parameters(), lr=self.optimizer_dict["lr"], weight_decay=1e-5)
    ...
    def save_model(self, fold):
        ...
        print("Saving model at {}".format("/home/nhattm/ToMoDL/ToMoDL/saved_models/" + self.save_path))
...
class UNetReconstructor(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log(
            "train/psnr_fbp", self.psnr(psnr_fbp_loss, range_max_min=[filtered_fs_rec.min(), filtered_fs_rec.max()])
        )
    ...
    def save_model(self):
        ...
        * Add method for model loading from checkpoint
            * Load names from versions and choose best k.
        """
        path = "/home/nhattm/ToMoDL/ToMoDL/saved_models/" + self.save_path + ".pth"
