...
train_name_modl = 'Optimization_Layers_PSNR_MODL_Test64'
train_name_SSIM = 'Optimization_Layers_SSIM_MODL_Test64'
...
def train_networks():
    ...
    for nLayer in nLayers:   
        ...
        if not model_MODL_path.is_file():
            ...
            with open(results_folder+train_name_modl+'LossLayers_Proj{}_nlay{}_epochs{}_K{}_lam{}_trnSize{}.pkl'.format(projection, nLayer, epochs, K, lam, train_factor), 'wb') as f:
...
def plot_testing():                                                                                                           
     ...
     with open(results_folder+train_name_modl+'nLayers_SSIM_PSNR.pkl', 'rb') as f:
     ...
     val_means = {k:[] for k in test_loss_dict[3].keys()}
 
     for nLayer, losses in test_loss_dict.items():
         ...
         for i, (loss_key, val_loss) in enumerate(losses.items()):
             ...
             if i<3:
                 ...
                 if nLayer==3:
                     ...
                     ax[0].errorbar(nLayer, val_loss.mean(), yerr = val_loss.std(), marker = 'h', fmt = '-',c = col[i], label = loss_key, alpha = alpha, capsize=capsize, elinewidth=elinewidth, markeredgewidth=markeredgewidth)
                 
                 else:
         
                     ax[0].errorbar(nLayer, val_loss.mean(), yerr = val_loss.std(), marker = 'h', fmt = '-',c = col[i], alpha = alpha, capsize=capsize, elinewidth=elinewidth, markeredgewidth=markeredgewidth)
                 ...
                 if nLayer == 3:
                     ...
                     ax[1].errorbar(nLayer, val_loss.mean(), yerr = val_loss.std(), marker = 'h', fmt = '-',c = col[i], label = loss_key, alpha = alpha, capsize=capsize, elinewidth=elinewidth, markeredgewidth=markeredgewidth)
                 
                 else:
                     
                     ax[1].errorbar(nLayer, val_loss.mean(), yerr = val_loss.std(),marker = 'h', fmt = '-', c = col[i], alpha = alpha, capsize=capsize, elinewidth=elinewidth, markeredgewidth=markeredgewidth)
                 i = i+3
 
             if nLayer == nLayers[-1]:
                 
                 if i<3:
                     ax[0].plot(nLayers, val_means[loss_key], c = col[i])
                
                 else:
     
                     ax[1].plot(nLayers, val_means[loss_key], c = col[i-3])
     ...
     fig.savefig(results_folder+'nLayers_Comparison_SSIM_PSNR.pdf', bbox_inches = 'tight')
...
def acquire_testing():
    ...
    for nLayer in nLayers:
        ...
        test_loss_dict[nLayer] = {'mse_modl': test_loss_modl, 'mse_fbp': test_loss_fbp, 'mse_ssim':test_loss_ssim, 'ssim_modl':ssim_modl_test, 'ssim_fbp':ssim_fbp_test, 'ssim_ssim':ssim_SSIM_test}    
        
        with open(results_folder+train_name_modl+'nLayers_SSIM_PSNR.pkl', 'wb') as f:
