...
img_size = 100
train_size = 0.7
train_name = 'Optimization_K_SSIM_MSE_Test65'
batch_size = 5

with open(results_folder+train_name+'LossKs_nlay{}_epochs{}_K{}_lam{}_trnSize{}.pkl'.format(nLayer, epochs, K, lam, train_size), 'rb') as f:
    train_info = pickle.load(f)
   
#%% Plot
fig, ax = plt.subplots(1,1, figsize = (8,6))

train = modutils.psnr(img_size, train_info[K]['train_mse'], batch_size)+10*np.log10(batch_size)
val = modutils.psnr(img_size,train_info[K]['val_mse'], batch_size)+10*np.log10(batch_size)
train_fbp = modutils.psnr(img_size,train_info[K]['train_fbp'], batch_size)+10*np.log10(batch_size)
