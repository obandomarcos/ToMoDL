    ...
    def reconstruct(self, sinogram: np.ndarray):
        ...
        if self.iradon_functor == None:
            try:
                ...
                self.iradon_functor = radon_thrad(image_size = self.resize_val, 
                                              thetas = self.angles_torch, 
                                              circle = self.clip_to_circle,
                                              det_count = np.ceil(np.sqrt(2)*self.resize_val).astype(int),
                                              filter = None if self.use_filter == False else 'ramp',
                                              device = device)        
            except:
                self.iradon_functor = None
        ...
        if self.rec_process == Rec_Modes.FBP_GPU.value:
            ...
            self.iradon_function = lambda sino: self.iradon_functor.filter_backprojection(torch.Tensor(sino)[None, None].to(device))[0, 0].cpu().numpy()
