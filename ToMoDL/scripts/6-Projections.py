...
from lightning.profiler import SimpleProfiler
import argparse
import numpy as np
...
def runs(testing_options):
    ...
    if use_default_model_dict == True:
        ...
        optimizer_dict = {"optimizer_name": "Adam", "lr": 1e-4}
    ...
    if use_default_trainer_dict == True:
        ...
        logger_dict = {"project": "deepopt", "entity": "omarcos", "log_model": True}
    ...
    if use_default_dataloader_dict == True:
        ...
        dataloader_dict = {
            "datasets_folder": where_am_i("datasets"),
            "number_volumes": 0,
    ...
    if "train_projections_kfold_ssim" in testing_options:
        ...
        for acc_factor in acceleration_factors:
            ...
            wandb.init(
                project=f"UNET Res - X{acc_factor}",
                save_code=True,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--train_projections_kfold_ssim", help="Train w/SSIM loss with optimal hyperparameters", action="store_true"
    )
    parser.add_argument(
        "--train_projections_kfold_psnr", help="Train w/PSNR loss with optimal hyperparameters", action="store_true"
    )
