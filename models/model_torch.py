...
class Aclass:
    ...
    def __init__(self, kw_dictionary):
        ...
        self.img_size = kw_dictionary.pop('img_size')
...
class OPTmodl(nn.Module):
  ...
  def forward(self, atb):
    ...
    for i in range(1,self.K+1):
        ...
        self.out['dc'+j] = inverse(self.AtA, rhs)
        
        torch.cuda.empty_cache()
        del rhs

    self.out['dc'+j] = normalize_image(self.out['dc'+j])    
    #print('output network max', self.out['dc'+j].max(), 'min', self.out['dc'+j].min()) 
    return self.out
  
   number_layers, K, n_angles, proj_num, image_size, mask, lam, results_folder, shared = True, useUnet = False, unet_options = None
