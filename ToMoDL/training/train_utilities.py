...
class TrainerSystem:
    ...
    def process_trainer_kwdictionary(self, kwdict):
        ...
        if self.use_accumulate_batches == True:
            ...
            self.lightning_trainer_dict["callbacks"] += [
                GradientAccumulationScheduler(scheduling={self.batch_accumulation_start_epoch: 2})
            ]
    ...
    def reinitialize_logger(self):
        ...
        if self.use_logger == True:
            ...
            if self.use_k_folding == True:
                ...
                self.logger_dict["name"] = (
                    f"x{self.acceleration_factor} - {self.loss_method} - K-Fold {self.current_fold}/{self.k_fold_max-1} - {self.image_size} size"
                )
    ...
    def process_dataloader_kwdictionary(self, kwdict):
        ...
        self.folders_datasets_list = [
            self.datasets_folder + "x{}/".format(self.acceleration_factor) + x
            for x in os.listdir(self.datasets_folder + "x{}".format(self.acceleration_factor))
        ]
    ...
    def print_check_datasets(self):
        ...
        train_val_datasets_folders = self.folders_datasets_list[
            : self.datasets_number - self.k_fold_number_datasets
        ].copy()
    ...
    def kfold_monitor(self, train_val_datasets, test_datasets):
        ...
        self.kfold_monitor_dict[self.current_fold] = {
            "k_fold": self.current_fold,
    ...
    def __parse_dataset_list(self, dataset_list):
        ...
        stats_dict = {
            "fish_part": self.__dict_counting(fish_part, "part"),
    ...
    def generate_K_folding_dataloader(self):
        ...
        if self.number_volumes * self.test_factor > 1:
            ...
            test_datasets_folders = self.folders_datasets_list[
                self.datasets_number - self.k_fold_number_datasets :
            ].copy()
        ...
        train_val_lengths = [
            int(len(train_val_datasets) * self.train_factor),
    ...
    def k_folding(self):
        ...
        for k_fold in range(self.k_fold_max):
            ...
            if (
                (self.restore_fold == True)
                and (self.acceleration_factor == self.acc_factor_restore)
                and (self.current_fold < self.fold_number_restore)
            ):

                print(
                    "Fold {} for acceleration factor x{} already done...".format(
                        self.current_fold, self.acceleration_factor
                    )
                )
