...
resnet_options_dict = {"number_layers": 8, "kernel_size": 3, "features": 64, "in_channels": 1, "out_channels": 1, "stride": 1, "use_batch_norm": True, "init_method": "xavier"}
...
# U-Net parameters
unet_dict = {"n_channels": 1, "n_classes": 1, "bilinear": True, "batch_norm": True, "batch_norm_inconv": True, "residual": False, "up_conv": False}

# TwIST parameters
twist_dictionary = {"number_projections": modl_dict["number_projections_total"], "lambda": 1e-2, "tolerance": 1e-4, "stop_criterion": 1, "verbose": 0, "initialization": 0, "max_iter": 10, "gpu": 0, "tau": 0.01}

# Training parameters
loss_dict = {"loss_name": "psnr", "psnr_loss": torch.nn.MSELoss(reduction="mean"), "ssim_loss": SSIM(data_range=1, size_average=True, channel=1), "msssim_loss": MSSSIM(kernel_size=1), "l1_loss": torch.nn.L1Loss(reduction="mean")}
...
logger_dict = {"project": "tomodl", "entity": "user", "log_model": True}

lightning_trainer_dict = {"max_epochs": 40, "log_every_n_steps": 10, "check_val_every_n_epoch": 1, "gradient_clip_val": 0.5, "accelerator": "gpu", "devices": 1, "fast_dev_run": False, "default_root_dir": model_folder}
